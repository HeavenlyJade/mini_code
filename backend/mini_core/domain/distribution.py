...
class DistributionGrade(EntityInt):
    ...
    self_ratio: float = field(default=None, metadata=dict(description='自购比例'))
    first_ratio: float = field(default=None, metadata=dict(description='一级分拥比例'))
    second_ratio: float = field(default=None, metadata=dict(description='二级分拥比例'))
